...
from getAssetForCell import getAssetForCell
from position import Position
from snake import Snake
from candyMap import CandyMap
import pygame
from constant import GAME_OVER_BACKGROUND_COLOR, DIFFICULTY, GAME_FIELD_BACKGROUND_COLOR, GAME_OVER_TEXT_COLOR, SNAKE_BODY_COLOR, WINDOW_SIZE_X, WINDOW_SIZE_Y
...
class GameEngine:
  ...
  def __makeSnakeIteration(self, direction: Direction):
    ...
    if doesHeadFacesCandy:
      candy: Candy = self.__candiesField.getCandyBy(self.__snake.headPosition)  # type: ignore
  ...
  def __renderCell(self, position: Position):
    ...
    pygame.draw.rect(self.__gameWindow, SNAKE_BODY_COLOR, [position.x, position.y, position.x + 1, position.x + 1])
    ...
  ...
  def __renderGameOverScreen(self):
    ...
    game_over_surface = my_font.render('GAME OVER', True, GAME_OVER_TEXT_COLOR)
  ...
  def __renderScore(self, choice, color, font, size):
    ...
    score_surface = score_font.render(f'Score : {self.__playerScore}', True, color)
